...
def getTrafficLines(image):
    ...
    lines = lineCascade.detectMultiScale(gray, 1.1, 4, 10, (1, 1),(6,6))
    lineID = 0
...
def testLines(lines, image, resultImage):
    for lineID in lines.keys():
        t_x = lines[lineID][0]
        t_y = lines[lineID][1]
        t_w = lines[lineID][2]
        t_h = lines[lineID][3]
        cv2.rectangle(resultImage, (t_x, t_y), (t_x + t_w, t_y + t_h), (0, 255, 255), 2)
...
def getPixelPerMetric(line, cnts, metric, resultImage, carLocation):
    ...
    if (lineContour == None):
        ...
        cv2.line(resultImage, (x,y), (carLocation[0],carLocation[1]+carLocation[3]), (255,0,0), 2)
        cv2.line(resultImage, (x,y), (x+w, y+h), (255,255,0), 2)
...
def trackMultipleObjects():
    ...
    while True:
        ...
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        cars = carCascade.detectMultiScale(gray, 1.1, 16, 18, (25, 25))
        ...
        # testLines(lineTracker, image, resultImage)
        ...
        for i in carLocation1.keys():
            if frameCounter % 1 == 0:
                ...
                if [x1, y1, w1, h1] != [x2, y2, w2, h2]:
                    ...
                    speed[i] = estimateSpeed([x1, y1, w1, h1], [x2, y2, w2, h2], lineTracker, image, cnts, resultImage)
